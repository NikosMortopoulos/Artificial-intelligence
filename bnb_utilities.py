...
def validate_cvrp_routes(
    instance,
    routes,
    require_exact_vehicle_count=True,
    verbose=False,
):
    n = instance["n"]
    Q = instance["Q"]
    ...
    K = int(math.ceil(total_demand / Q))
    ...
    for route_idx, route in enumerate(routes):
        ...
        # Ignore empty depot-only routes if any appear.
        if len(customers) == 0:
            continue
    ...
    if require_exact_vehicle_count:
        if len(normalized_routes) != K:
            if verbose:
                print(
                    f"Invalid number of routes: got {len(normalized_routes)}, "
